#!/usr/bin/env python3
"""
University of Wisconsin-Milwaukee course catalog scraper.
URL: catalog.uwm.edu/courses/{dept}/
HTML: button.courseblocktitle.noindent > strong "DEPT NUM Title"
      + p.courseblockdesc.noindent (description)
"""
...
BASE_URL = "https://catalog.uwm.edu"
...
STEM = {"actsci", "astron", "bme", "bms", "chem", "compsci", "eleceng", "frshwtr",
        "geog", "math", "matleng", "mecheng", "mthstat", "neuro", "physics", "stat"}
HUMANITIES = {"afric", "amllc", "arabic", "art", "arthist", "celtic", "chinese",
              "classic", "complit", "dance", "english", "eap", "filmstd", "film",
              "french", "german", "greek", "hebrew", "hist", "hmong", "italian",
              "japan", "jewish", "korean", "latin", "linguis", "music", "musperf",
              "philos", "polish", "portugs", "religst", "russian", "spanish",
              "theatre", "trnsltn", "wlc"}
SOCIAL = {"afas", "ais", "anthro", "comsdis", "commun", "compst", "ces", "couns",
          "currins", "econ", "educ", "ethnic", "global", "intlst", "jams", "lacs",
          "lacusl", "latinx", "lgbt", "peacest", "psych", "ph", "sociol",
          "urbplan", "wgs"}
MEDICAL = {"atrain", "fitwell", "foodbev", "hca", "hi", "kin", "nurs", "nutr",
           "occthpy", "pt", "therrec"}
PROFESSIONAL = {"apc", "arch", "busmgmt", "dmi", "dac", "econ", "hca", "infost", "nonprof"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        # Title is in button.courseblocktitle or p.courseblocktitle
        title_el = block.find("button", class_="courseblocktitle")
        if not title_el:
            title_el = block.find("p", class_="courseblocktitle")
        if not title_el:
            continue
        strong = title_el.find("strong")
        raw = (strong or title_el).get_text(" ", strip=True)
        # "ACTSCI 290 Introduction to Actuarial Science"
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\s+\([0-9].*\))?\s*$", raw)
        if not m:
            continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        title = m.group(3).strip().rstrip(".")
        title = re.sub(r"\s+\([0-9].*\)\s*$", "", title).strip().rstrip(".")

        # Description in p.courseblockdesc.noindent
...
def main():
    ...
    print(f"=== University of Wisconsin-Milwaukee Course Catalog Scraper ===")
