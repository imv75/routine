#!/usr/bin/env python3
"""
University of Alaska Fairbanks course catalog scraper.
URL: catalog.uaf.edu/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle "ACCT F261X      Principles of Financial Accounting3 Credits"
      + p.courseblockdesc
Note: title and credit hours are concatenated without separator in courseblocktitle text.
"""
...
BASE_URL = "https://catalog.uaf.edu"
...
STEM = {"biol", "chem", "cs", "ce", "ee", "enve", "geol", "math", "me", "mse",
        "phys", "stat", "es", "ine", "mine", "pete"}
HUMANITIES = {"anth", "art", "engl", "fren", "germ", "hist", "hum", "japn",
              "musi", "phil", "russ", "span", "thea", "writ"}
SOCIAL = {"anth", "comm", "econ", "educ", "geog", "just", "pols", "psyc",
          "soci", "socw", "sw"}
MEDICAL = {"biol", "hlth", "kine", "nurs", "nutr", "vet"}
PROFESSIONAL = {"acct", "bus", "fin", "mgmt", "mktg", "law", "cj", "crj"}
...
def classify_level(num):
    try:
        # UAF uses F-prefixed numbers like F261X; strip non-digits then parse
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # "ACCT F261X      Principles of Financial Accounting3 Credits"
        # Credits may be concatenated directly: "...AccountingX Credits" or "...Accounting X Credits"
        # First extract dept and num
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([A-Z]?\d+\w*)\s+(.+)$", raw)
        if not m:
            continue
        ...
        # Strip trailing credits: "3 Credits", "1-3 Credits", etc.
        # Pattern handles cases like "Title3 Credits" (no space before digit) or "Title 3 Credits"
        title = re.sub(r"\s*\d+(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s+Credits?\.?\s*$", "", rest, flags=re.IGNORECASE).strip()
        # If credits are concatenated without space, strip them too
        title = re.sub(r"\d+(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s+Credits?\.?\s*$", "", title, flags=re.IGNORECASE).strip()
        title = title.rstrip(".")
...
def main():
    ...
    print(f"=== University of Alaska Fairbanks Course Catalog Scraper ===")
