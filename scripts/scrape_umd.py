#!/usr/bin/env python3
"""
University of Maryland College Park course catalog scraper.
URL: academiccatalog.umd.edu/undergraduate/approved-courses/{dept}/
     academiccatalog.umd.edu/graduate/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "ENGL101 Title (N Credits)"
      + p.courseblockdesc
"""
...
BASE_URL = "https://academiccatalog.umd.edu"
...
STEM = {"amsc", "aosc", "bioe", "biol", "bsci", "chem", "chbe", "cmsc", "enae",
        "ence", "enco", "enee", "enma", "enme", "ennu", "enpm", "enre",
        "ense", "entm", "envs", "fmsc", "geog", "geol", "math", "mieh",
        "nacs", "neur", "phys", "psyc", "stat", "zool"}
HUMANITIES = {"aaas", "amst", "anth", "arab", "arth", "artt", "chin",
              "clas", "cmlt", "engl", "film", "fren", "germ", "grek",
              "hebr", "hist", "ital", "japn", "jour", "kora",
              "latn", "ling", "mlaw", "musc", "pers", "phil",
              "port", "russ", "slav", "span", "thet", "wgss"}
SOCIAL = {"aast", "anth", "comm", "crim", "econ", "educ",
          "geog", "gvpt", "hsml", "jrnl", "larc",
          "plsc", "psyc", "socy", "socw", "ursp"}
MEDICAL = {"biom", "epib", "fmsc", "hlth", "knes",
           "mieh", "nacs", "neur", "nfsm", "nurs",
           "octh", "phsc", "phsi", "phth", "psyc", "ptrk", "sphb"}
PROFESSIONAL = {"aasp", "abre", "agnr", "agst", "ansc", "arch",
                "arec", "arhu", "army", "bmgt", "buac", "busi",
                "cpsp", "edci", "edpl", "educ", "fire", "hmxd",
                "honr", "inst", "lbsc", "lwst", "mgmt",
                "mktg", "nfsc", "plsc", "prst",
                "rsmt", "smgt", "tlpl", "univ"}
...
def parse_block(block):
    ...
    # Format: "ENGL101 Academic Writing (3 Credits)"
    # Dept code is letters, num follows immediately (no space)
    m = re.match(r"([A-Z][A-Z0-9]*[A-Z])(\d[\w]*)\s+(.+?)(?:\s*\(\d|$)", title_text)
    if not m:
        # Try with space: "CMSC 131 ..."
        m2 = re.match(r"([A-Z]+)\s+(\d[\w]*)\s+(.+?)(?:\s*\(|$)", title_text)
        if not m2:
            return None
        dept, num, title = m2.group(1), m2.group(2), m2.group(3).strip()
    else:
        dept, num, title = m.group(1), m.group(2), m.group(3).strip()
...
def get_depts(session, catalog="undergraduate"):
    if catalog == "undergraduate":
        url = f"{BASE_URL}/undergraduate/approved-courses/"
        pattern = r"/approved-courses/([^/]+)/"
    else:
        url = f"{BASE_URL}/graduate/courses/"
        pattern = r"/graduate/courses/([^/]+)/"
    try:
        ...
        depts = []
        for l in links:
            m = re.search(pattern, l["href"])
            if m:
                depts.append(m.group(1))
        return list(dict.fromkeys(depts))
    except Exception as e:
        print(f"  ERROR fetching {catalog} dept list: {e}")
        return []


def scrape_dept(session, dept, catalog):
    if catalog == "undergraduate":
        url = f"{BASE_URL}/undergraduate/approved-courses/{dept}/"
    else:
        url = f"{BASE_URL}/graduate/courses/{dept}/"
    try:
        r = session.get(url, timeout=25)
        if r.status_code != 200:
            return []
...
def main():
    ...
    print(f"=== University of Maryland Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    for catalog in ["undergraduate", "graduate"]:
        depts = get_depts(session, catalog)
        ...
        for dept in depts:
            ...
            for c in courses:
                ...
                if key not in seen:
                    ...
                    new += 1
            if not courses:
                failed.append(f"{dept}({catalog[:4]})")
            elif new:
                print(f"  {dept.upper()} ({catalog[:4]}): {new} courses")
            time.sleep(0.2)
