#!/usr/bin/env python3
"""
Messiah University course catalog scraper.
URL: messiah.smartcatalogiq.com/en/2025-2026/{ug-catalog/undergraduate-course-offerings|grad-catalog/course-offerings}/{dept}/{level}/{course}
...
BASE_URL = "https://messiah.smartcatalogiq.com"
...
STEM = {"aphs", "arcg", "biol", "chem", "csci", "engn", "envs", "math", "nurs", "phys",
        "stat", "hlth", "info", "data"}
HUMANITIES = {"art", "arth", "comm", "engl", "fren", "ger", "grek", "hist", "hum",
              "lat", "lang", "musi", "phil", "reli", "span", "thea", "wgst", "bib"}
SOCIAL = {"anth", "cjus", "econ", "educ", "pols", "psyc", "soci", "socw", "sw", "blst"}
MEDICAL = {"kine", "nutr", "atht", "nurs", "ot", "pt"}
PROFESSIONAL = {"acct", "buad", "busa", "fin", "mgmt", "mktg", "mba"}
...
def classify_level(section):
    return "graduate" if "graduate" in section.lower() else "undergraduate"
...
def get_dept_paths(session, catalog_path, section_name):
    url = f"{BASE_URL}{catalog_path}/{section_name}"
    r = session.get(url, timeout=25)
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{catalog_path}/{section_name}/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))


def get_course_urls(session, dept_path, catalog_path, section_name):
    url = f"{BASE_URL}{dept_path}/"
    r = session.get(url, timeout=25)
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{catalog_path}/{section_name}/[a-z][a-z0-9\-]+/\d+/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))
...
def main():
    ...
    print(f"=== Messiah University Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    catalog_sections = [
        ("/en/2025-2026/undergraduate-catalog", "undergraduate-course-offerings", "undergraduate"),
        ("/en/2025-2026/graduate-catalog", "course-offerings", "graduate"),
    ]

    all_course_items = []
    for catalog_path, section_name, level in catalog_sections:
        depts = get_dept_paths(session, catalog_path, section_name)
        print(f"  {level}: {len(depts)} depts")
        for dp in depts:
            urls = get_course_urls(session, dp, catalog_path, section_name)
