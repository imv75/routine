#!/usr/bin/env python3
"""
University of Findlay course catalog scraper.
URL: findlay.smartcatalogiq.com/en/current/{undergraduate|graduate}-catalog/courses/{dept}/{level}/{course}
...
BASE_URL = "https://findlay.smartcatalogiq.com"
...
STEM = {"anhs", "ansc", "biol", "chem", "csci", "envs", "geol", "math", "phys", "stat",
        "nurs", "phar", "scie"}
HUMANITIES = {"art", "comm", "engl", "fren", "ger", "hist", "ling", "musi", "phil", "reli",
              "span", "thea", "wgst", "blmc"}
SOCIAL = {"anth", "cjus", "econ", "educ", "psyc", "soci", "socw", "pols", "psych", "addc"}
MEDICAL = {"hlth", "kine", "nurs", "nutr", "phar", "athl"}
PROFESSIONAL = {"acct", "buad", "fin", "mgmt", "mktg", "bsad", "bsem"}
...
def get_course_urls(session, dept_path):
    ...
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(r"^/en/current/[a-z\-]+-catalog/courses/[a-z][a-z0-9\-]+/\d+/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))
...
def main():
    ...
    print(f"=== University of Findlay Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    catalog_sections = [
        ("/en/current/undergraduate-catalog", "undergraduate"),
        ("/en/current/graduate-catalog", "graduate"),
    ]
