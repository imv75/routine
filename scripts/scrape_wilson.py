#!/usr/bin/env python3
"""
Wilson College course catalog scraper.
URL: wilson.smartcatalogiq.com/en/current/catalog/course-listings/{dept}/{level}/{course}
...
BASE_URL = "https://wilson.smartcatalogiq.com"
CATALOG_PATH = "/en/current/catalog"
...
STEM = {"bio", "chm", "csc", "envs", "math", "phys", "stat", "dat", "ans"}
...
def get_dept_paths(session):
    url = f"{BASE_URL}{CATALOG_PATH}/course-listings"
    r = session.get(url, timeout=25)
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_PATH}/course-listings/[a-z][a-z0-9\-]+$")
...
def get_course_urls(session, dept_path):
    ...
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_PATH}/course-listings/[a-z][a-z0-9\-]+/\d+/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))
...
def main():
    ...
    print(f"=== Wilson College Course Catalog Scraper ===")
