#!/usr/bin/env python3
"""
University of Nebraska at Kearney course catalog scraper.
URL: catalog.unk.edu/{undergraduate|graduate}/courses/{dept}/
HTML: div.courseblock > p.noindent > strong "ACCT 126 – Title     N credit hour"
      + text content of div.courseblock (description)
"""
...
BASE_URL = "https://catalog.unk.edu"
...
STEM = {"biol", "chem", "cs", "ce", "ee", "envs", "geol", "math", "phys", "stat",
        "it", "indm", "emst"}
HUMANITIES = {"art", "comm", "engl", "fren", "germ", "hist", "musi", "phil",
              "span", "thea", "writ"}
SOCIAL = {"anth", "econ", "educ", "geog", "pols", "psyc", "soci", "socw", "sw"}
MEDICAL = {"biol", "hlth", "kine", "nurs", "nutr"}
PROFESSIONAL = {"acct", "agbs", "bsad", "fin", "law", "mgmt", "mktg"}
...
def get_dept_slugs(session, level):
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            m = re.match(rf"^/{level}/courses/([a-z][a-z0-9]*)/?$", a["href"])
            if m:
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        title_p = block.find("p", class_="noindent")
        if not title_p:
            continue
        ...
        # "ACCT 126 – First Year Seminar     1 credit hour"
        # Em dash or regular dash between number and title
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s*[–-]\s*(.+?)(?:\s+\d+\.?\d*\s+credit.*)?$", raw, re.IGNORECASE)
        if not m:
            continue
        ...
        title = re.sub(r"\s+\d+\.?\d*\s+credit.*$", "", title, flags=re.IGNORECASE).strip().rstrip(".")

        # Description: text in courseblock after the title p
        title_p.extract()
...
def main():
    ...
    print(f"=== University of Nebraska at Kearney Course Catalog Scraper ===")
