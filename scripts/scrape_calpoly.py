#!/usr/bin/env python3
"""
California Polytechnic State University (Cal Poly SLO) course catalog scraper.
URL: catalog.calpoly.edu/courses/{dept}/
HTML: div.courseblock > div.courseblocktitle > strong "AGB 1101\nTitle\n(3 units)"
      + div.courseblock__description
"""
...
STEM = {"aero", "ag", "arce", "bae", "bio", "bmed", "bot", "chem", "ce", "csc",
        "ece", "env", "ersc", "fsn", "geol", "math", "me", "mse", "nrm", "phys",
        "ss", "stat", "wrc"}
HUMANITIES = {"ant", "arch", "art", "engl", "fren", "germ", "grc", "hist",
              "ital", "japn", "lat", "ling", "musi", "phil", "port", "rels",
              "russ", "span", "sth", "thea"}
SOCIAL = {"ant", "comm", "econ", "educ", "geog", "gs", "pols", "psyc", "soc",
          "socw", "wgs"}
MEDICAL = {"bio", "bmed", "fsn", "kine", "nurs", "nutr"}
PROFESSIONAL = {"agb", "agc", "aged", "asci", "bmed", "bus", "cm", "fnce",
                "jtpr", "law", "mgt", "mkt", "reb"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        if not strong:
            continue
        # Text is multi-line: "AGB 1101\nIntroduction to Agribusiness\n(3 units)"
        lines = [l.strip() for l in strong.get_text().split("\n") if l.strip()]
        if len(lines) < 2:
            continue
        code_line = lines[0]  # "AGB 1101"
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)", code_line)
        if not m:
            continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        # Title is everything between code line and units line
        title_lines = []
        for line in lines[1:]:
            if re.match(r"^\(\d+", line):  # "(3 units)" pattern
                break
            title_lines.append(line)
        title = " ".join(title_lines).strip().rstrip(".")

        desc_div = block.find("div", class_="courseblock__description")
...
def main():
    ...
    print(f"=== California Polytechnic State University Course Catalog Scraper ===")
