#!/usr/bin/env python3
"""
University of Missouri course catalog scraper.
URL: catalog.missouri.edu/courseofferings/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "DEPT NUM: Title"
      + p.courseblockdesc
"""
...
STEM = {"bio_sc", "bioc", "biostat", "chem", "cmpsci", "div_st", "earth",
        "ece", "econ", "engr", "env_sc", "fish", "for_sc", "geo_sci", "infotc",
        "math", "micr", "mn_en", "nat_r", "phy_sc", "physcs", "plnt_sc",
        "psych", "stat", "vetsc", "wldlf"}
HUMANITIES = {"arab", "art", "art_h", "class", "clas", "col", "complit", "dance",
              "engl", "film", "fren", "ger", "gk", "greek", "hist", "ital",
              "japan", "jour", "lat", "ling", "music", "musc", "phil", "port",
              "rel_st", "russ", "slav", "span", "theatre", "writ"}
SOCIAL = {"anthro", "com_r", "crim", "ec", "educ", "econ", "geog", "hlth",
          "human", "ir", "mgt", "pols", "pub_h", "r_a_r", "soc", "soc_w",
          "socio", "st", "urb_r", "wgs"}
MEDICAL = {"anat_c", "biol", "bm", "bme", "clin_c", "derm", "em", "fam_med",
           "gen", "gen_c", "hlth", "imc", "immun", "med", "mebi", "neuro",
           "nurs", "ob_gyn", "onco", "opht", "ortho", "path", "peds",
           "phm", "phys_t", "psyc", "rad", "rehab", "surg", "urol"}
PROFESSIONAL = {"acctcy", "agr", "blaw", "bus", "ed_l", "ed_p", "educ",
                "espe", "fin", "hr", "hm", "ib", "journ", "law", "lm",
                "mgt", "mktg", "pub_a", "sp_ed", "tc"}
...
def classify_level(num):
    try:
        ...
        return "graduate" if n >= 7000 else "undergraduate"
    except Exception:
...
def get_dept_slugs(session):
    url = f"{BASE_URL}/courseofferings/"
    try:
        ...
        for a in soup.find_all("a", href=True):
            m = re.match(r"/courseofferings/([^/]+)/?$", a["href"])
            if m and m.group(1):
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # Format: "ANTHRO 1000:  Introduction to Anthropology..."
...
def main():
    ...
    print(f"=== University of Missouri Course Catalog Scraper ===")
    ...
    for slug in slugs:
        url = f"{BASE_URL}/courseofferings/{slug}/"
        try:
            r = session.get(url, timeout=25, allow_redirects=True)
            if r.status_code != 200:
                failed.append(slug)
