#!/usr/bin/env python3
"""
Hartwick College course catalog scraper.
URL: hartwick.smartcatalogiq.com/en/2024-2025/catalog/course-descriptions/{dept}/{level}/{course}
...
UNIVERSITY = "hartwick"
CATALOG_YEAR = "2025"
CATALOG_LABEL = "2024-2025"
BASE_URL = "https://hartwick.smartcatalogiq.com"
CATALOG_PATH = "/en/2024-2025/catalog"
...
STEM = {"biol", "chem", "csci", "envs", "geol", "math", "phys", "stat", "nurs"}
HUMANITIES = {"acco", "art", "arth", "engl", "fren", "ger", "grek", "hist", "hum",
              "lat", "musi", "phil", "reli", "span", "thea", "wgst", "comm", "film"}
SOCIAL = {"anth", "econ", "educ", "pols", "psyc", "soci", "socw", "geog", "cjus"}
MEDICAL = {"hlth", "kine", "nurs", "nutr", "ath"}
...
def get_dept_paths(session):
    url = f"{BASE_URL}{CATALOG_PATH}/course-descriptions"
    r = session.get(url, timeout=25)
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_PATH}/course-descriptions/[a-z][a-z0-9\-]+$")
...
def get_course_urls(session, dept_path):
    ...
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_PATH}/course-descriptions/[a-z][a-z0-9\-]+/\d+/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))
...
def main():
    ...
    print(f"=== Hartwick College Course Catalog Scraper ===")
