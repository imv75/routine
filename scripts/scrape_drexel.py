#!/usr/bin/env python3
"""
Drexel University course catalog scraper.
URL: catalog.drexel.edu/coursedescriptions/quarter/{undergrad|grad}/{slug}/
HTML: div.courseblock > p.courseblocktitle > strong > span.cdspacing[0] (dept+num) + span.cdspacing[1] (title)
      + p.courseblockdesc
"""
...
STEM = {"biol", "bmes", "chem", "chm", "cive", "cs", "cse", "eam", "ecec",
        "ece", "enve", "envs", "eny", "mat", "math", "mse", "mse", "mee",
        "mem", "mte", "nfs", "phys", "psy", "stat"}
HUMANITIES = {"arth", "artp", "chin", "clas", "clst", "coml", "comm", "danc",
              "engl", "fmst", "fmtv", "fren", "germ", "grek", "hist", "ital",
              "jpns", "latn", "ling", "musc", "phil", "port", "russ", "span",
              "thtr", "writ"}
SOCIAL = {"anth", "com", "crim", "econ", "educ", "geog", "glbs", "pols",
          "psyc", "soc", "socw", "sts", "urbs"}
MEDICAL = {"bios", "dnce", "envh", "hlth", "hpm", "hsad", "hsci", "nfs",
           "nurs", "nutr", "octh", "phth", "pub", "pubh", "reha"}
PROFESSIONAL = {"acct", "busn", "busn", "desn", "dsre", "entp", "fin", "gmap",
                "hmgt", "intb", "law", "lawm", "mba", "mgmt", "mktg", "oper",
                "orgb", "real", "scm", "tax"}
...
def get_dept_slugs(session, level):
    url = f"{BASE_URL}/coursedescriptions/quarter/{level}/"
    try:
        ...
        slugs = []
        pattern = f"/coursedescriptions/quarter/{level}/"
        for a in soup.find_all("a", href=True):
            href = a["href"]
            if href.startswith(pattern) and href != pattern:
                m = re.search(rf"/coursedescriptions/quarter/{level}/([^/]+)/", href)
                if m:
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # Try span.cdspacing first
        spans = strong.find_all("span", class_=lambda c: c and "cdspacing" in c)
        if len(spans) >= 2:
            code_text = spans[0].get_text(strip=True)
            ...
            # Fallback: parse from full strong text
            title_text = strong.get_text(" ", strip=True)
            # Format: "DEPT CODE  Title N.N Credits"
            m = re.match(r"([A-Z][A-Z0-9]*)\s+([A-Z]?\d[\w]*)\s+(.+?)(?:\s+[\d.]+\s+Credits?|$)", title_text)
            if not m:
                m = re.match(r"([A-Z][A-Z0-9]*)\s+([A-Z]?\d[\w]*)\s+(.+)", title_text)
                if not m:
                    continue
            dept = m.group(1)
            num = m.group(2)
            title = m.group(3).strip()
            # Clean up trailing credit info
...
def main():
    ...
    print(f"=== Drexel University Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    for level in ["undergrad", "grad"]:
        slugs = get_dept_slugs(session, level)
        print(f"Scraping {len(slugs)} {level} departments...")
        for slug in slugs:
            url = f"{BASE_URL}/coursedescriptions/quarter/{level}/{slug}/"
            try:
                r = session.get(url, timeout=25)
                if r.status_code != 200:
                    failed.append(f"{slug}({level})")
                    continue
                ...
                for dept, num, title, desc in parsed:
                    ...
                    if key not in seen:
                        ...
                        new += 1
                if new:
                    print(f"  {slug.upper()} ({level}): {new} courses")
                elif not parsed:
                    failed.append(f"{slug}({level})")
            except Exception as e:
