#!/usr/bin/env python3
"""
Utah Valley University course catalog scraper.
URL: catalog.uvu.edu/courses/{dept}/
HTML: div.courseblock > div.cols.noindent "ACC 1110.  Financial Accounting I.  (1.5 Credits)"
      + div.noindent (description)
"""
...
BASE_URL = "https://catalog.uvu.edu"
...
STEM = {"biol", "chem", "cs", "ce", "ee", "envs", "geol", "math", "me",
        "phys", "stat", "it", "cit", "engr", "mech"}
HUMANITIES = {"art", "arth", "comm", "engl", "fren", "germ", "hist", "hum",
              "musi", "phil", "span", "thea", "writ", "lang"}
SOCIAL = {"anth", "econ", "educ", "geog", "pols", "psyc", "soci", "socw", "sw", "geog"}
MEDICAL = {"biol", "dent", "hlth", "kine", "nurs", "nutr", "dh"}
PROFESSIONAL = {"acc", "bus", "fin", "mgmt", "mktg", "law", "cj", "crj", "mba"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        # Header: div.cols.noindent "ACC 1110.  Financial Accounting I.  (1.5 Credits)"
        header_div = block.find("div", class_=lambda c: c and "cols" in (c if isinstance(c, str) else " ".join(c)))
        if not header_div:
            header_div = block.find("div")
        if not header_div:
            continue

        raw = header_div.get_text(" ", strip=True)
        # "ACC 1110.  Financial Accounting I.  (1.5 Credits)"
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+?)(?:\.\s+\([0-9].*\)|\.?\s*\([0-9].*\))?\s*$", raw)
        if not m:
            m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+)", raw)
            if not m:
                # Try without period after course number
        ...
        # Remove trailing credit info if still present
        title = re.sub(r"\s+\([0-9].*\)\s*$", "", title).strip().rstrip(".")

        # Description: look for div.noindent that is NOT the header
        desc = ""
        for d in block.find_all("div", class_="noindent"):
            if d == header_div:
                continue
...
def main():
    ...
    print(f"=== Utah Valley University Course Catalog Scraper ===")
