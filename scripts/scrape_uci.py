#!/usr/bin/env python3
"""
University of California Irvine Course Catalog Scraper.
Source: catalogue.uci.edu/allcourses/{dept}/
Format: CourseleafCMS detail format with span.detail-code, span.detail-title, div.courseblockextra
"""
...
STEM_DEPTS = {"math", "chem", "physics", "bio sci", "ece", "cs", "eecs",
              "mae", "cbems", "cbe", "bme", "stats", "ics", "compsci",
              "informatics", "cse", "engr", "eecs", "bats", "anatomy",
              "biochem", "molbio", "mcb", "neurobio", "bio", "earthss"}
HUMANITIES_DEPTS = {"english", "history", "philosophy", "classics", "art his",
                    "dance", "drama", "music", "film & media", "humanities",
                    "writing", "comp lit", "cmlang", "french", "german", "spanish",
                    "italian", "japanese", "chinese", "korean", "persian",
                    "arabic", "hebrew", "art", "arts", "wrt"}
SOCIAL_DEPTS = {"anthropology", "criminology", "economics", "education",
                "global cultures", "iss", "pols", "psychology", "sociology",
                "womens studies", "gender & sexuality", "ethnic studies",
                "afam", "asianam", "chicano/latino", "lgbtqs", "wgss"}
MEDICAL_DEPTS = {"anatomy", "bme", "clinical psychology", "nursing", "pharmacology",
                 "physical therapy", "public health", "medicine", "bats"}
PROFESSIONAL_DEPTS = {"accounting", "business", "mgmt", "finance", "mba",
                      "ped", "law", "education"}
...
def parse_uci_block(block):
    ...
    title_text = title_span.get_text(" ", strip=True).strip(" .—-") if title_span else ""
    
    m = re.match(r"^(.*?)\s+([\d]+[A-Z]?)\s*$", code_text.strip())
    if not m:
        return None
    ...
    # Description: first div.courseblockextra
    desc_divs = block.find_all("div", class_="courseblockextra")
    desc = ""
    for d in desc_divs:
        ...
        if t and len(t) > 10:
            desc = t
            break
    desc = re.sub(r"^Course Description:\s*", "", desc, flags=re.IGNORECASE)
...
def get_departments():
    ...
    r = s.get("https://catalogue.uci.edu/allcourses/", timeout=20)
    soup = BeautifulSoup(r.text, "html.parser")
    pc = soup.find("div", class_="page_content") or soup
    depts = []
    for a in pc.find_all("a", href=True):
        m = re.match(r"^/allcourses/([^/]+)/?$", a["href"])
        if m and m.group(1) != "":
...
def scrape_dept(dept_slug):
    s = requests.Session()
    s.headers["User-Agent"] = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/120"
    url = f"https://catalogue.uci.edu/allcourses/{dept_slug}/"
    try:
...
def scrape_uci():
    output_dir = "/home/user/routine/data/uci"
    os.makedirs(output_dir, exist_ok=True)
    print("=== UC Irvine Course Catalog Scraper ===")
    ...
    csv_path = f"{output_dir}/uci_2026.csv"
    ...
    summary = {
        "university": "uci", "academic_year": "2026",
        "academic_year_label": "2026-2027",
        "source": "catalogue.uci.edu/allcourses/",
        "total_courses": total,
    ...
    with open(f"{output_dir}/uci_summary.json", "w") as f:
