#!/usr/bin/env python3
"""
Texas State University course catalog scraper.
URL: mycatalog.txstate.edu/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle "ACC 2301. Title." + p.courseblockdesc
"""
...
BASE_URL = "https://mycatalog.txstate.edu"
...
STEM = {"biol", "chem", "cs", "ce", "ee", "envs", "geol", "math", "me", "mse",
        "phys", "stat", "cis", "cosc"}
HUMANITIES = {"aas", "amst", "arab", "art", "artf", "arth", "chin", "engl",
              "fren", "germ", "grek", "hist", "ital", "japn", "lat", "ling",
              "musi", "phil", "port", "rel", "russ", "span", "thea", "wgss"}
SOCIAL = {"anth", "comm", "crim", "econ", "educ", "geog", "pols", "psyc",
          "soci", "socw", "urp"}
MEDICAL = {"biol", "hlth", "kine", "nurs", "nutr", "pt", "rhs"}
PROFESSIONAL = {"acc", "fin", "mis", "mgmt", "mktg", "hmgt", "law", "ba", "blaw"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+?)\.?\s*$", raw)
        if not m:
            continue
...
def main():
    ...
    print(f"=== Texas State University Course Catalog Scraper ===")
