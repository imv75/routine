#!/usr/bin/env python3
"""
George Mason University course catalog scraper.
URL: catalog.gmu.edu/courses/{dept}/
HTML: div.courseblock > div.courseblocktitle > strong.cb_code "ACCT 203:" + em.cb_title "Title."
      + div.courseblockdesc
"""
...
STEM = {"astr", "beng", "biod", "binf", "biol", "chem", "cds", "ce", "cs",
        "ece", "envs", "geog", "geol", "math", "me", "mse", "neurosci",
        "phys", "psyc", "stat", "sys"}
HUMANITIES = {"afam", "arab", "arth", "avt", "chin", "clas", "comm", "crwr",
              "engl", "film", "fren", "germ", "grek", "hist", "ital", "japn",
              "lat", "ling", "musi", "phil", "port", "rel", "russ", "span", "thea"}
SOCIAL = {"afam", "anth", "comm", "econ", "educ", "geog", "govt", "ints",
          "pols", "psyc", "soc", "socw"}
MEDICAL = {"biol", "kine", "nurs", "nutr", "reh", "shs"}
PROFESSIONAL = {"acct", "ait", "amgt", "bis", "bus", "fin", "hmgt", "law",
                "mgmt", "mktg", "pub", "pubp"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        if not title_div:
            continue
        code_strong = title_div.find("strong", class_="cb_code")
        title_em = title_div.find("em", class_="cb_title")

        if not code_strong:
            continue
        code_text = code_strong.get_text(strip=True).rstrip(":")
        # Format: "ACCT 203" (or "ACCT 203/ACCT 503" for cross-listed)
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([\d\w]+)", code_text)
        if not m:
            continue
        dept = m.group(1).strip()
        num = m.group(2).strip()

        title = title_em.get_text(strip=True).rstrip(".") if title_em else ""
        if not title:
            # Try to get title from the div text after code
            remaining = title_div.get_text(" ", strip=True)
            remaining = re.sub(re.escape(code_strong.get_text(strip=True)), "", remaining).strip()
            remaining = re.sub(r"^\s*:\s*", "", remaining)
            title = re.sub(r"\.\s*\d+\s+credits?.*$", "", remaining, flags=re.IGNORECASE).strip()
...
def main():
    ...
    print(f"=== George Mason University Course Catalog Scraper ===")
