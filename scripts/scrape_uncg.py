#!/usr/bin/env python3
"""
UNC Greensboro course catalog scraper.
URL: catalog.uncg.edu/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle.noindent "ACC 201  Financial Accounting  3"
...
STEM = {"bsc", "bio", "che", "csc", "cen", "ens", "geo", "mat", "mso", "phy", "sta"}
HUMANITIES = {"aas", "afs", "amst", "ark", "art", "arh", "chi", "cla", "com", "enl",
              "fra", "ger", "gre", "his", "ita", "jap", "lat", "lin", "msa", "mus",
              "phi", "por", "rel", "rus", "spa", "thea", "wmst"}
SOCIAL = {"aas", "anth", "com", "cri", "eco", "edu", "geo", "psc", "psy", "soc", "socw", "wms"}
MEDICAL = {"bio", "kin", "nrs", "ntr", "phm"}
PROFESSIONAL = {"acc", "bus", "fin", "hmg", "inf", "isb", "law", "mba", "mgt", "mkt"}
...
def get_dept_slugs(session):
    r = session.get(f"{BASE_URL}/courses/", timeout=25)
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # "ACC 201  Financial Accounting  3" (trailing number = credits)
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\s+\d+(?:\.\d+)?\s*$|\s*$)", raw)
        if not m:
            m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+)", raw)
            if not m:
                continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        title = m.group(3).strip().rstrip(".")
        # Remove trailing credits digits
...
def main():
    ...
    all_courses, seen, failed = [], set(), []
    print(f"=== UNC Greensboro Course Catalog Scraper ===\nCatalog year: {CATALOG_LABEL}")
        ...
        try:
            ...
            for dept, num, title, desc in parsed:
                ...
                if key not in seen:
                    ...
                    all_courses.append({
                        "university": UNIVERSITY, "academic_year": CATALOG_YEAR, "academic_year_label": CATALOG_LABEL,
                        "department_code": dept, "course_number": num, "title": title, "description": desc,
                        "broad_area": classify_area(dept), "level": classify_level(num),
                        ...
                        "cross_listed": False, "deduplicated": True,
                    })
                    new += 1
            if new: print(f"  {slug.upper()}: {new} courses")
            elif not parsed: failed.append(slug)
        except Exception as e:
            print(f"  ERROR {slug}: {e}"); failed.append(slug)
        time.sleep(0.2)

    total = len(all_courses)
    print(f"\nTotal unique courses: {total}")
    if failed: print(f"Failed ({len(failed)}): {', '.join(failed[:20])}")
    ...
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields); w.writeheader(); w.writerows(all_courses)
    ...
    area_counts = {}
    for c in all_courses: area_counts[c["broad_area"]] = area_counts.get(c["broad_area"], 0) + 1
    summary = {"university": UNIVERSITY, "academic_year": CATALOG_YEAR, "academic_year_label": CATALOG_LABEL, "total_courses": total,
