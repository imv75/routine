#!/usr/bin/env python3
"""
Arkansas Tech University course catalog scraper.
URL: catalog.atu.edu/courses/{dept}/
HTML: div.courseblock > div.cols.noindent
        span.detail-code > strong "ACCT 2000"
        span.detail-title > strong "Accounting Principles I Lab"
      + div.courseblockextra.noindent (description)
"""
...
BASE_URL = "https://catalog.atu.edu"
...
STEM = {"biol", "chem", "cs", "ce", "ee", "envs", "geol", "math", "me",
        "phys", "stat", "it", "engr"}
HUMANITIES = {"art", "comm", "engl", "fren", "germ", "hist", "hum",
              "musi", "phil", "span", "thea", "writ"}
SOCIAL = {"anth", "econ", "educ", "geog", "pols", "psyc", "soci", "socw", "sw"}
MEDICAL = {"biol", "dent", "hlth", "kine", "nurs", "nutr"}
PROFESSIONAL = {"acct", "bus", "fin", "law", "mgmt", "mktg"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        code_span = block.find("span", class_=lambda c: c and "detail-code" in (c if isinstance(c, str) else " ".join(c)))
        title_span = block.find("span", class_=lambda c: c and "detail-title" in (c if isinstance(c, str) else " ".join(c)))
        ...
        desc = ""
        for d in block.find_all("div", class_=lambda c: c and "courseblockextra" in (c if isinstance(c, str) else " ".join(c))):
...
def main():
    ...
    print(f"=== Arkansas Tech University Course Catalog Scraper ===")
