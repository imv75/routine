#!/usr/bin/env python3
"""
University of Montana course catalog scraper.
URL: catalog.umt.edu/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "ACTG 101 - Accounting Procedures I. 3 Credits."
      + p.courseblockdesc
"""
...
BASE_URL = "https://catalog.umt.edu"
...
STEM = {"bchm", "biol", "chem", "cs", "ce", "ee", "envs", "geol", "math", "me",
        "mse", "phys", "stat", "for", "geos"}
HUMANITIES = {"anth", "art", "chin", "engl", "fren", "germ", "grek", "hist",
              "hum", "jpns", "lat", "ling", "musi", "nast", "phil", "russ",
              "span", "thea", "writ"}
SOCIAL = {"anth", "comm", "econ", "educ", "geog", "pols", "psyc", "soci", "socw", "sw"}
MEDICAL = {"biol", "dent", "hlth", "kin", "nurs", "nutr", "pt", "phy"}
PROFESSIONAL = {"actg", "ba", "fin", "law", "mgmt", "mktg"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s*[-–]\s*(.+?)(?:\.\s*\d+.*Credits?.*)?\.?\s*$", raw)
        if not m:
            # Try without dash (fallback)
            m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\.\s*\d+.*Credits?.*)?\.?\s*$", raw)
            if not m:
                continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        title = m.group(3).strip().rstrip(".")
        # Remove trailing credit info
        title = re.sub(r"\.\s*\d+.*Credits?.*$", "", title, flags=re.IGNORECASE).strip().rstrip(".")
...
def main():
    ...
    print(f"=== University of Montana Course Catalog Scraper ===")
