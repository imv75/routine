#!/usr/bin/env python3
"""
North Park University course catalog scraper.
URL: northpark.smartcatalogiq.com/en/2025-2026/north-park-university-2025-2026-academic-catalog/courses/{dept}/{level}/{course}
...
BASE_URL = "https://northpark.smartcatalogiq.com"
CATALOG_PATH = "/en/2025-2026/north-park-university-2025-2026-academic-catalog"
...
STEM = {"biol", "chem", "csci", "math", "phys", "stat", "envs", "nurs", "data"}
HUMANITIES = {"arab", "art", "amus", "bts", "cms", "engl", "fren", "grek", "hist",
              "lat", "ling", "musi", "phil", "scan", "span", "thea", "wgst"}
SOCIAL = {"anth", "comm", "econ", "educ", "pols", "psyc", "soci", "socw", "sw",
          "cjus", "glst"}
MEDICAL = {"hlth", "kine", "nurs", "nutr"}
PROFESSIONAL = {"acct", "badm", "bse", "fin", "mgmt", "mktg"}
...
def get_course_urls(session, dept_path):
    ...
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_PATH}/courses/[a-z][a-z0-9\-]+/\d+/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))
...
def main():
    ...
    print(f"=== North Park University Course Catalog Scraper ===")
