#!/usr/bin/env python3
"""
Alverno College course catalog scraper.
URL: catalog.alverno.edu/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle.noindent "A-100 Title (N credits)"
      + p.courseblockdesc
"""
...
BASE_URL = "https://catalog.alverno.edu"
...
STEM = {"bi", "ch", "cs", "ma", "ns", "ph"}
HUMANITIES = {"a", "en", "fr", "ge", "hi", "mu", "re", "sp", "th"}
SOCIAL = {"co", "ec", "py", "so"}
MEDICAL = {"nu", "ot"}
PROFESSIONAL = {"bu", "ed", "mg"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # "A-100 Community Art/Design Midsemester (0 credits)"
        # "AL-645 Mentoring: Strength Professional Practice (3 credits)"
        m = re.match(r"([A-Z][A-Z0-9]*)-(\d+\w*)\s+(.+?)(?:\s+\([\d\-]+\s+credit.*\))?\s*$", raw)
        if not m:
            # Try without dash
            m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\s+\([\d\-]+\s+credit.*\))?\s*$", raw)
            if not m:
                continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        title = m.group(3).strip().rstrip(".")
        # Strip trailing credit info if regex didn't
        title = re.sub(r"\s+\([\d\-]+\s+credit.*\)\s*$", "", title, flags=re.IGNORECASE).strip().rstrip(".")
...
def main():
    ...
    print(f"=== Alverno College Course Catalog Scraper ===")
