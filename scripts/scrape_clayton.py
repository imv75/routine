#!/usr/bin/env python3
"""
Clayton State University course catalog scraper.
URL: catalog.clayton.edu/courses/{undergrad|graduate}-level-courses/{dept}/
HTML: div.cols.noindent > span.detail-code "ACCT 2101." + span.detail-title "Title."
...
BASE_URL = "https://catalog.clayton.edu"
...
STEM = {"biol", "chem", "cs", "csci", "math", "phys", "stat", "envs", "astr", "info"}
HUMANITIES = {"afam", "art", "engl", "film", "fren", "germ", "hist", "ling", "musi",
              "phil", "span", "thea"}
SOCIAL = {"anth", "comm", "crim", "econ", "educ", "geog", "pols", "psyc", "soci", "socw", "sw"}
MEDICAL = {"hlth", "kine", "nurs", "nutr"}
PROFESSIONAL = {"acct", "admn", "busa", "fin", "law", "mgmt", "mktg"}
...
def get_dept_slugs(session, level):
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            m = re.match(rf"^/courses/{level}/([a-z][a-z0-9]*)/?$", a["href"])
            if m:
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # Description: p.courseblockextra inside div.noindent
        desc = ""
...
def main():
    ...
    print(f"=== Clayton State University Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    all_slugs = []
    for level in ["undergraduate-level-courses", "graduate-level-courses"]:
    ...
    for level, slug in all_slugs:
        url = f"{BASE_URL}/courses/{level}/{slug}/"
        try:
            r = session.get(url, timeout=25)
            if r.status_code != 200:
            ...
            for dept, num, title, desc in parsed:
                ...
                if key not in seen:
                    ...
                    new += 1
            if new:
                print(f"  {level[:4]}/{slug.upper()}: {new} courses")
            elif not parsed:
                failed.append(f"{level}/{slug}")
        except Exception as e:
