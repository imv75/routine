#!/usr/bin/env python3
"""
Montana State University course catalog scraper.
URL: catalog.montana.edu/coursedescriptions/{dept}/
HTML: div.courseblock > p.courseblocktitle "ACTG 101  Accounting Procedures I: 4 Credits (4 Lec)"
      + p.courseblockdesc
"""
...
UNIVERSITY = "montanastate"
CATALOG_YEAR = "2026"
CATALOG_LABEL = "2026-2027"
BASE_URL = "https://catalog.montana.edu"
...
STEM = {"aero", "amth", "biob", "biom", "bios", "chem", "cosc", "ce", "ecns",
        "ece", "eosc", "fish", "for", "geol", "math", "me", "mse", "mbi",
        "phys", "plsc", "stat", "wild", "wsci"}
HUMANITIES = {"amst", "arab", "chin", "engl", "fren", "germ", "grek", "hist",
              "ital", "japn", "lat", "ling", "musi", "nasx", "phil", "rel", "russ",
              "sign", "span", "thtr", "writ"}
SOCIAL = {"anth", "comm", "ecns", "educ", "geog", "hstr", "just", "nais", "psyc",
          "soci", "socw"}
MEDICAL = {"biob", "biom", "hlth", "kine", "nurs", "nutr"}
PROFESSIONAL = {"actg", "agbe", "abus", "bmgt", "fin", "hosp", "law", "mgmt", "mktg"}
...
def get_dept_slugs(session):
    url = f"{BASE_URL}/coursedescriptions/"
    try:
        ...
        for a in soup.find_all("a", href=True):
            m = re.match(r"^/coursedescriptions/([a-z][a-z0-9]*)/?$", a["href"])
            if m:
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # "ACTG 101  Long Title Here  3 Credits"
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\s*:\s*\d+\s+Credits?.*|\s+\d+\s+Credits?.*)?$", raw)
        if not m:
            continue
...
def main():
    ...
    print(f"=== Montana State University Course Catalog Scraper ===")
    ...
    for slug in slugs:
        url = f"{BASE_URL}/coursedescriptions/{slug}/"
        try:
            r = session.get(url, timeout=25)
            if r.status_code != 200:
                failed.append(slug)
