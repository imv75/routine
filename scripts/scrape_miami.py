#!/usr/bin/env python3
"""
University of Miami course catalog scraper.
URL: bulletin.miami.edu/courses-az/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "DEPT 100.  Title (TYPE).  N Credit Hours."
      + p.courseblockdesc
"""
...
BASE_URL = "https://bulletin.miami.edu"
...
STEM = {"atm", "bil", "bme", "bmb", "bst", "cab", "cet", "chem", "chm", "cis",
        "csc", "ece", "eco", "epi", "fsc", "geo", "ggy", "isc", "mth", "mar",
        "msc", "nsc", "phy", "psy", "sta", "sys"}
HUMANITIES = {"aas", "ams", "arb", "arc", "art", "arh", "clc", "cla", "cls",
              "cpn", "dss", "eng", "fre", "ger", "gre", "grk", "his", "ita",
              "jpn", "kor", "lat", "lin", "mll", "mus", "phi", "por", "rel",
              "rus", "spa", "thr", "wri"}
SOCIAL = {"aas", "ams", "ant", "apy", "com", "ccs", "cri", "eco", "edu",
          "geog", "gsy", "hst", "ibe", "ins", "jrn", "pol", "ppa", "psc",
          "soc", "sow", "urb", "wst"}
MEDICAL = {"ahs", "bmb", "bst", "cab", "cer", "den", "gen", "gme", "gyn",
           "hlt", "hms", "kin", "med", "msc", "nur", "occ", "pat", "phd",
           "phy", "pub", "rad", "reh", "sur"}
PROFESSIONAL = {"acc", "ais", "arc", "bus", "bsl", "bte", "fin", "gde", "hmg",
                "iba", "law", "mba", "mgmt", "mkt", "mls", "msc", "rea", "scm",
                "tax"}
...
def get_dept_slugs(session):
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            m = re.match(r"^/courses-az/([a-z]+)/$", a["href"])
            if m:
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # Format: "APY 100.  Introduction to Forensic Investigation (EXP).  3 Credit Hours."
        # or: "APY 500.  Advanced Topics.  3 Credit Hours."
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+?)(?:\s*\([A-Z]+\))?\.\s+[\d\-]+ Credit", title_text)
        if not m:
        ...
        # Clean trailing credit info and type codes from title
        title = re.sub(r"\s*\([A-Z,]+\)\s*$", "", title).strip()
        title = re.sub(r"\.\s*[\d\-]+ Credit.*$", "", title).strip()
...
def main():
    ...
    print(f"=== University of Miami Course Catalog Scraper ===")
    ...
    for slug in slugs:
        url = f"{BASE_URL}/courses-az/{slug}/"
        try:
            r = session.get(url, timeout=25)
            if r.status_code != 200:
                failed.append(slug)
