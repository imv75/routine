#!/usr/bin/env python3
"""
Youngstown State University course catalog scraper.
URL: catalog.ysu.edu/courses/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "ACCT 1503    Essentials of Accounting    3 s.h."
      + p.courseblockdesc
"""
...
BASE_URL = "https://catalog.ysu.edu"
...
STEM = {"biol", "chem", "cs", "ce", "chee", "ee", "envs", "geol", "math", "me",
        "mfge", "phys", "stat", "it"}
HUMANITIES = {"afst", "art", "arth", "engl", "fren", "germ", "grek", "hist",
              "ital", "lat", "musi", "phil", "rels", "span", "thea", "wgst"}
SOCIAL = {"anth", "comm", "crim", "econ", "educ", "geog", "pols", "psyc",
          "soci", "socw", "sw"}
MEDICAL = {"biol", "dent", "hlth", "kine", "nurs", "nutr", "rhs"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # "ACCT 1503    Essentials of Accounting    3 s.h."
        # Fields are separated by multiple spaces; trailing credit: "N s.h."
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\s+\d+\.?\d*\s+s\.h\.?)?\s*$", raw)
        if not m:
            continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        title = m.group(3).strip().rstrip(".")
        # Remove trailing credit hours if still present
        title = re.sub(r"\s+\d+\.?\d*\s+s\.h\.?\s*$", "", title).strip().rstrip(".")
...
def main():
    ...
    print(f"=== Youngstown State University Course Catalog Scraper ===")
