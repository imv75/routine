#!/usr/bin/env python3
"""
Marietta College course catalog scraper.
URL: marietta.smartcatalogiq.com/en/name/2025-2026-college-catalog/{ug|grad}-course-descriptions/{dept}/{level}/{course}
...
BASE_URL = "https://marietta.smartcatalogiq.com"
CATALOG_BASE = "/en/name/2025-2026-college-catalog"
...
STEM = {"biol", "chem", "csci", "envs", "geol", "math", "phys", "stat", "astr", "data"}
HUMANITIES = {"acrl", "art", "arth", "asl", "clas", "engl", "fren", "ger", "grek",
              "hist", "hum", "lat", "musi", "phil", "reli", "span", "thea", "comm",
              "film"}
SOCIAL = {"anth", "cjus", "econ", "educ", "pols", "psyc", "soci", "socw", "geog"}
MEDICAL = {"hlth", "kine", "nurs", "nutr", "athl", "exsc", "pasp"}
PROFESSIONAL = {"acct", "buad", "fin", "mgmt", "mktg", "mba", "coun", "masl"}
...
def get_dept_paths(session, section):
    url = f"{BASE_URL}{CATALOG_BASE}/{section}"
    r = session.get(url, timeout=25)
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_BASE}/{section}/[a-z][a-z0-9\-]+$")
...
def get_course_urls(session, dept_path, section):
    ...
    soup = BeautifulSoup(r.text, "html.parser")
    links = [a["href"] for a in soup.find_all("a", href=True)]
    pattern = re.compile(rf"^{CATALOG_BASE}/{section}/[a-z][a-z0-9\-]+/\d+/[a-z][a-z0-9\-]+$")
    return list(dict.fromkeys(h for h in links if pattern.match(h)))
...
def main():
    ...
    print(f"=== Marietta College Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    sections = [
        ("undergraduate-course-descriptions", "undergraduate"),
        ("graduate-course-descriptions", "graduate"),
    ]
    ...
    def fetch_one(args):
        path, level = args
        return fetch_course(session, path, "undergraduate-course-descriptions" if level == "undergraduate" else "graduate-course-descriptions")
