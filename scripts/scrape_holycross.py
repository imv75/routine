#!/usr/bin/env python3
"""
College of the Holy Cross course catalog scraper.
URL: catalog.holycross.edu/courses/{dept}/
HTML: div.cols.noindent > span.detail-code "AFST 110 —" + span.detail-title "Title"
      + div.noindent > p.courseblockextra (description)
"""
...
UNIVERSITY = "holycross"
CATALOG_YEAR = "2026"
CATALOG_LABEL = "2026-2027"
BASE_URL = "https://catalog.holycross.edu"
...
STEM = {"biol", "chem", "cs", "math", "phys", "stat", "envs", "neur"}
HUMANITIES = {"acip", "afst", "art", "arth", "chin", "clas", "engl", "fren", "germ",
              "grek", "hist", "ital", "jpns", "lat", "ling", "medi", "musi", "phil",
              "port", "russ", "span", "thea", "wgst", "relg"}
SOCIAL = {"anth", "comm", "econ", "educ", "pols", "psyc", "soci", "socw"}
MEDICAL = {"kine"}
PROFESSIONAL = {"acct", "fin"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        if not code_span or not title_span:
            continue
        # "AFST 110 —" or "AFST 110 -"
        code_text = code_span.get_text(strip=True).rstrip("—-").strip()
        ...
        # Description in p.courseblockextra (first one that looks like description)
        desc = ""
...
def main():
    ...
    print(f"=== College of the Holy Cross Course Catalog Scraper ===")
