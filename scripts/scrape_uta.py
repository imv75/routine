#!/usr/bin/env python3
"""
University of Texas at Arlington course catalog scraper.
URL: catalog.uta.edu/coursedescriptions/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "ACCT 2301.  Title.  N Hours."
      + p.courseblockdesc
"""
...
BASE_URL = "https://catalog.uta.edu"
...
STEM = {"ae", "astr", "biol", "bcmn", "biol", "chem", "cse", "ce", "ee", "enve",
        "geol", "ie", "math", "mse", "me", "phy", "stat", "nurs"}
HUMANITIES = {"aast", "arab", "arch", "art", "asl", "engl", "fren", "germ",
              "grek", "hist", "hons", "ital", "jpns", "lat", "ling", "musi",
              "phil", "port", "reli", "span", "thea", "wgss"}
SOCIAL = {"aast", "anth", "comm", "crim", "econ", "educ", "geog", "glbs",
          "pols", "psyc", "soci", "socw", "urbn", "wgss"}
MEDICAL = {"biol", "kine", "nurs", "ot", "pt", "pub", "rha"}
PROFESSIONAL = {"acct", "advt", "busa", "fin", "hmgt", "info", "law", "mba",
                "mgmt", "mktg", "real", "ta"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # Format: "ACCT 2301.  Principles of Accounting I.  3 Hours."
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+?)(?:\.\s*[\d\-]+\s+Hours?\.?.*|$)", title_text, re.DOTALL)
        if not m:
...
def main():
    ...
    print(f"=== University of Texas at Arlington Course Catalog Scraper ===")
