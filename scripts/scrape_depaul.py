#!/usr/bin/env python3
"""
DePaul University course catalog scraper.
URL: catalog.depaul.edu/course-descriptions/{dept}/
HTML: p.courseblocktitle.noindent "ACC\xa0101 | TITLE | N quarter hours"
...
BASE_URL = "https://catalog.depaul.edu"
...
STEM = {"csc", "mat", "che", "bio", "phy", "sta", "cit", "ece", "mec", "env", "ecg"}
HUMANITIES = {"afr", "ara", "art", "ath", "chi", "cla", "enc", "eng", "fre", "ger",
              "gre", "his", "ita", "jpn", "lat", "lin", "mus", "phi", "por", "rel",
              "rus", "spa", "the", "wgst"}
SOCIAL = {"ant", "com", "eco", "edu", "geo", "int", "pol", "psy", "soc", "sow", "sw"}
MEDICAL = {"hlt", "kin", "nrs", "nut"}
PROFESSIONAL = {"acc", "bus", "fin", "law", "man", "mkt", "mgt", "acc"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        if not title_p:
            continue
        raw = title_p.get_text(" ", strip=True).replace("\xa0", " ")
        # "ACC 101 | INTRODUCTION TO ACCOUNTING I | 4 quarter hours"
        parts = [p.strip() for p in raw.split("|")]
        if len(parts) < 2:
            continue
        code_part = parts[0].strip()
        title_part = parts[1].strip()
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)", code_part)
        if not m:
            continue
...
def main():
    ...
    print(f"=== DePaul University Course Catalog Scraper ===")
