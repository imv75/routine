#!/usr/bin/env python3
"""
Biola University course catalog scraper.
URL: catalog.biola.edu/courses/{dept}/
HTML: p.courseblocktitle "DEPT\xa0NUM -  Title Credits N"
...
BASE_URL = "https://catalog.biola.edu"
...
STEM = {"biol", "chem", "cs", "math", "phys", "stat", "engi", "envs"}
HUMANITIES = {"arab", "art", "arth", "chin", "engl", "fren", "germ", "grek",
              "hist", "lat", "ling", "musi", "phil", "span", "thea"}
SOCIAL = {"anth", "comm", "econ", "educ", "pols", "psyc", "soci", "socw", "sw"}
MEDICAL = {"hlth", "kine", "nurs", "nutr"}
PROFESSIONAL = {"acct", "fin", "mgmt", "mktg", "bu"}
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        # "ASLG 100 -  Elementary American Sign Language Credits 4"
        # non-breaking space between dept and num, dash before title
        raw = raw.replace("\xa0", " ")
        m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s*-+\s*(.+?)(?:\s+Credits?\s+[\d\.]+)?\s*$", raw, re.IGNORECASE)
        if not m:
            m = re.match(r"([A-Z][A-Z0-9]*)\s+(\d+\w*)\s+(.+?)(?:\s+Credits?\s+[\d\.]+)?\s*$", raw, re.IGNORECASE)
            if not m:
                continue
        dept = m.group(1).strip()
        num = m.group(2).strip()
        title = m.group(3).strip().rstrip(".")
        title = re.sub(r"\s+Credits?\s+[\d\.]+\s*$", "", title, flags=re.IGNORECASE).strip().rstrip(".")
...
def main():
    ...
    print(f"=== Biola University Course Catalog Scraper ===")
