#!/usr/bin/env python3
"""
University of Texas at San Antonio course catalog scraper.
URL: catalog.utsa.edu/{undergrad|grad}/coursedescriptions/{dept}/
HTML: div.courseblock > p.courseblocktitle > strong "ACC 2003.  Title.  (3-0) 3 Credit Hours."
      + p.courseblockdesc
"""
...
BASE_URL = "https://catalog.utsa.edu"
...
STEM = {"bio", "chem", "cs", "ce", "ee", "envs", "geol", "math", "me", "mse",
        "phy", "stat", "bme", "bioc", "aiml"}
HUMANITIES = {"ais", "aas", "asl", "ahc", "amst", "arab", "art", "arh", "chin",
              "engl", "fren", "germ", "grek", "hist", "ital", "japn", "lat",
              "ling", "musi", "phil", "port", "rel", "span", "thtr"}
SOCIAL = {"anth", "comm", "crim", "econ", "educ", "geog", "govt", "pols",
          "psyc", "soci", "socw", "urb", "wgs"}
MEDICAL = {"bio", "kine", "nurs", "hsc", "pt", "ot"}
PROFESSIONAL = {"acc", "acct", "bus", "fin", "mis", "mgt", "mkt", "real", "hmgt"}
...
def get_dept_slugs(session, level, path_level):
    url = f"{BASE_URL}/{path_level}/coursedescriptions/"
    try:
        ...
        for a in soup.find_all("a", href=True):
            m = re.match(rf"^/{path_level}/coursedescriptions/([a-z][a-z0-9]*)/?$", a["href"])
            if m:
...
def parse_dept_page(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        ...
        m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+?)(?:\.\s+[\(\d].*|\.?\s*$)", raw)
        if not m:
            m = re.match(r"([A-Z][A-Z0-9]*)\s+([\w]+)\.\s+(.+)", raw)
            if not m:
                continue
...
def main():
    ...
    print(f"=== University of Texas at San Antonio Course Catalog Scraper ===")
    print(f"Catalog year: {CATALOG_LABEL}")

    undergrad_slugs = get_dept_slugs(session, "undergraduate", "undergraduate")
    grad_slugs = get_dept_slugs(session, "graduate", "graduate")
    print(f"Undergraduate depts: {len(undergrad_slugs)}, Graduate depts: {len(grad_slugs)}")

    dept_list = [(f"{BASE_URL}/undergraduate/coursedescriptions/{s}/", s) for s in undergrad_slugs]
    dept_list += [(f"{BASE_URL}/graduate/coursedescriptions/{s}/", s) for s in grad_slugs]
    print(f"Total pages: {len(dept_list)}")
