#!/usr/bin/env python3
"""
Wake Forest University course catalog scraper.
URL: bulletin.wfu.edu/courses-az/{dept}/
HTML: div.courseblock > p.courseblocktitle.noindent > strong "DEPT NUM.  Title.  (N h)"
...
BASE_URL = "https://bulletin.wfu.edu"
...
HUMANITIES_DEPTS = {
    "aas", "aes", "afs", "arb", "art", "bhm", "chi", "cla", "div", "eng",
    "fln", "frn", "ger", "gre", "heb", "his", "hum", "ita", "jst", "lat",
    "mus", "phi", "por", "rel", "rus", "spa", "the", "wgs",
}
SOCIAL_DEPTS = {
    "ant", "com", "eco", "edu", "geo", "gst", "psc", "psy", "soc", "sw",
    "wgs", "urb",
}
STEM_DEPTS = {
    "bch", "bio", "bmb", "che", "csc", "ees", "mth", "neu", "phy", "sta",
    "ens",
}
MEDICAL_DEPTS = {
    "bmd", "bms", "cns", "mda", "nsc", "nsb", "phy", "pub",
}
PROFESSIONAL_DEPTS = {
    "acc", "bus", "fin", "lgl", "law", "mba", "mgt", "mkt",
}
...
def get_dept_links(session):
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if href.startswith("/courses-az/") and href != "/courses-az/":
...
def parse_courses(html_text):
    ...
    for block in soup.find_all("div", class_="courseblock"):
        # Title: p.courseblocktitle or p with class containing courseblocktitle
        title_p = block.find("p", class_=lambda c: c and "courseblocktitle" in c)
        if not title_p:
            continue
        ...
        # Description: p.courseblockdesc
        desc_p = block.find("p", class_=lambda c: c and "courseblockdesc" in c)
        desc = desc_p.get_text(" ", strip=True) if desc_p else ""

        # Parse strong_text: "DEPT NUM.  Title.  (N h)" or "DEPT NUM.  Title."
        # Pattern: WORD DIGITS.  rest
        m = re.match(r"^([A-Z]{2,6})\s+(\d{1,4}[A-Z]?)\.\s+(.+?)(?:\.\s*\(\S+\))?\s*\.?\s*$",
                     strong_text)
        if not m:
            # Fallback
            ...
            num = parts[1].rstrip(".")
            title = " ".join(parts[2:])
            title = re.sub(r"\s*\.\s*\([\d\w\s\-]+\)\s*\.?\s*$", "", title).strip()
            title = title.rstrip(".")
        else:
            dept = m.group(1)
            num = m.group(2)
            title = m.group(3).strip().rstrip(".")
            # Remove trailing credits like "(3 h)" at end of title
            title = re.sub(r"\s*\.\s*$", "", title)
            title = re.sub(r"\s*\([\d\w\s\-]+\)\s*$", "", title).strip()
...
def main():
    ...
    print(f"=== Wake Forest University Course Catalog Scraper ===")
